...
def sinusoidal_positional_encoding(n_positions: int, dim: int) -> torch.Tensor:
    """Fixed sine/cosine table (n_positions, dim), Vaswani et al. 2017.
    Vendored from braindecode master (PR #1106 dependency) for py3.10 compat:
...
class SleepFMStager(nn.Module):
    ...
    def from_pretrained(cls, *args, **kwargs):
        ...
        (local re-implementation for py3.10: safetensors load, no HF hub mixins)."""
        from safetensors import safe_open
        from huggingface_hub import hf_hub_download
        repo = kwargs.pop("pretrained_model_name_or_path", None) or cls._HF_DEFAULT_REPO
        init_kw = dict(n_outputs=5, n_chans=3, n_times=3840, sfreq=128)
        init_kw.update({k: kwargs.pop(k) for k in
                        ("n_outputs", "n_chans", "n_times", "sfreq") if k in kwargs})
        model = cls(**init_kw)
